...
greedy_assignment=[[] for i in range(n_tasks)]  # starting with blank assignment

st = [0 for _ in range(n_tasks)] # hash-set to store which tasks are already assigned
cnt = n_tasks # Count of number of tasks left to assign atleast one agent to

for i in range(n_agents):
    temp_assign=[]
    
    for j in range(n_tasks):
        if(cnt!=0 and st[j] == 1): # If the j-th task is already assigned
            temp_assign.append( -1 ) # Assigning -1 to the task to avoid assigning agents to it
            continue

        greedy_assignment[j].append(random_agent_permutation[i]) # inserting i-th agent into j-th task
        temp_assign.append( solution_value(greedy_assignment) )  # calculating solution value
    ...
    if(cnt!=0): # If there are still tasks left to assign agents to
        st[np.argmax(temp_assign)] = 1
        cnt -= 1
...
print("Time taken by greedy approach: ", greedy_time, " seconds")