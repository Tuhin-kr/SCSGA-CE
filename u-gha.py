...
cnt = n_tasks # Count of number of tasks left to assign agents to

for i in range(n_agents):
    temp_assign=[]
    
    for j in range(n_tasks):
        if(st[j] == 1): # If the j-th task is already assigned
    ...
    st[np.argmax(temp_assign)] = 1 # Marking the task as assigned
    cnt -= 1 # Decrementing the count of tasks left to assign agents to

    if(cnt == 0): # If all tasks are assigned atleast one agent , then reset the hash-set and cnt variable
        st = [0 for _ in range(n_tasks)]
        cnt = n_tasks
